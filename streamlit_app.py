...
user_api_key = st.sidebar.text_input(
    ...
    placeholder="openAIのAPIキーを入力して下さい",
    type="password",
)

uploaded_file = st.sidebar.file_uploader("学習データPDFのアップロード", type="pdf")
...
if user_api_key and uploaded_file:
    ...
    if "generated" not in st.session_state:
        st.session_state["generated"] = [
            "読み込んだ学習データPDFに関して回答します。（" + uploaded_file.name + "）"
        ]

    if "past" not in st.session_state:
        st.session_state["past"] = ["質問しますので、回答をお願いします。"]
    ...
    with container:
        with st.form(key="my_form", clear_on_submit=True):
            user_input = st.text_input(
                "質問：",
                placeholder="学習データPDFに関する質問を入力してください。",
                key="input",
            )
